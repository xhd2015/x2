...
class FileOperation{
protected:
	typedef X2fsUtil::FileNode FileNode;
public:
...
{
	FileNode *tempCur=curNode,*temp=NULL;
...
//		如果..超过了根目录，报错
		...
		else if(p->compare("..")==0)
		{
			...
			FileNode *temp=util.locatePath(tempCur, p->c_str());
			if(temp==NULL)
			{
...
{
	util.seterrno(X2fsUtil::ERROR_NOERR);
...
void FileOperation::mkdir(const string& dir)
{
	...
	if(util.geterrno() == X2fsUtil::ERROR_FILE_ALREDY_EXIST)
	{
...
void FileOperation::refreshCurrentPath()
{
	...
	X2fsUtil::FileTree tree=util.getFileTree();
	...
	while(p!=head)
	{
		curPath.insert(curPath.begin(),util.getFileName(p));
