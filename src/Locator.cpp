...
{
	if(_HowStart!=Locator::IGNORE && Util::sign(t.getStart() - this->p->getStart()) != _HowStart)return false;
	if(_HowLength!=Locator::IGNORE && Util::sign(t.getLimit() - this->p->getStart()) != _HowLength)return false;
	return true;
}
